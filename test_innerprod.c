...
int main()
{
  ...
  printf("%f, %f\n", innerprod(vector, vector), innerprod_simd(vector_simd, vector_simd));

  double res = 0.0;
  double res_simd = 0.0;

  start_t = rdtsc();
  for (j = 0; j < 1000; j++) res += innerprod(vector, vector);
  end_t = rdtsc();
  ...
  start_t = rdtsc();
  for (j = 0; j < 1000; j++) res_simd += innerprod_simd(vector_simd, vector_simd);
  end_t = rdtsc();
